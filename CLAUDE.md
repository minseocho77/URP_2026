...
## URP 확장 — week1-4 산출물 (하이브리드 UDE 로드맵)
`week1-2.md`, `wekk3-4.md` 사양 기반. **무결성 원칙**: 각 문서가 "직접 구현"으로
지정한 물리/loss/마스킹/adaptive 코어는 `NotImplementedError` + 가이드로 남기고,
plumbing(상수·데이터로딩·적분·학습루프·검증)만 채운다. **이 TODO 들을 임의로
채우지 말 것** — 학생의 학습·방어 근거다.

| 파일 | 주차 | 역할 | 비어있는 코어(TODO) |
|---|---|---|---|
| `mechanistic_model.py` | 1 | 등온 10상태 forward + 5개 검증 | `reaction_rates`, `rhs` |
| `greybox_ode.py` | 2 | grey-box 파라미터 추정 + loss 인프라 | `rates`, `forward`, `loss_fn`, `load_targets` |
| `nn_closure.py` | 3 | NN 닫힘항 Rθ | 물리제약, `distill_from_greybox` |
| `hybrid_ude.py` | 3 | Hybrid UDE (rates→Rθ) | `forward` balance, warm-start, 항별 grad 로깅 |
| `missing_label_experiment.py` | 4 | 결측 라벨 곡선 | structured 마스킹, `train_model_fn` 연결 |
| `LOSS_TUNING.md` | 3-4 | λ 튜닝 트래커 | (표 값 채우기 + adaptive weighting) |

- 상태 정식화: `x=[C_g,1..5, C_l,1..5]` (등온). 상수는 논문 Table2/Table4 하드코딩.
- week2+ 는 **torch/torchdiffeq 필요**. Windows 260자 경로제한으로 torch 설치가
  실패하면 관리자 PowerShell 에서 `LongPathsEnabled=1` 설정 후 재설치 (requirements.txt 참고).
- 실행: `python mechanistic_model.py` 는 torch 없이 동작(코어 미구현 시 안내 출력).
